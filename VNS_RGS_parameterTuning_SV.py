...
parameterTuningInstance_SV = load_workbook('parameterTuningInstance_SV.xlsx')
sheetRequests = parameterTuningInstance_SV["Requests"]

noOfNodes = 100
noOfRequests = 200  # 200 requests
requests = []
data_images = []
...
sheetPT = bookResults.create_sheet('parameterTuning_SV')

kmaxArray = [6, 8, 10]
percentArray = [0.25, 0.5, 1]
iterations = 20

for j in range(2, len(kmaxArray)):
    kmax = kmaxArray[j]
    print("kmax is:", kmax)

    for PI in range(1, len(percentArray)):
        ...
        for i in range(iterations):
            ...
            sheetPT.cell(row=j * len(percentArray) * iterations + PI * iterations + i + 2, column=3).value = i
            sheetPT.cell(row=j * len(percentArray) * iterations + PI * iterations + i + 2, column=4).value = str(route)
            sheetPT.cell(row=j * len(percentArray) * iterations + PI * iterations + i + 2, column=5).value = str(routeRequest)
            sheetPT.cell(row=j * len(percentArray) * iterations + PI * iterations + i + 2, column=6).value = finalReward - finalCost
            sheetPT.cell(row=j * len(percentArray) * iterations + PI * iterations + i + 2, column=7).value = numGS
            sheetPT.cell(row=j * len(percentArray) * iterations + PI * iterations + i + 2, column=8).value = start_time_VNS
            sheetPT.cell(row=j * len(percentArray) * iterations + PI * iterations + i + 2, column=9).value = end_time_VNS
            sheetPT.cell(row=j * len(percentArray) * iterations + PI * iterations + i + 2, column=10).value = end_time_VNS - start_time_VNS
            sheetPT.cell(row=j * len(percentArray) * iterations + PI * iterations + i + 2, column=11).value = timeToFindBestSolution
