...
noOfNodes = 100
noOfRequests = 104  # 100 requests
requests = []
data_images = []
maxCPUtime = 180  # 180 seconds
...
driverNo = [1, 2, 3, 4]  # randomly chosen between 1 and 200
capacity = 4
noOfGS = 0
...
kmax = 10
percentNumber = 0.25
monteCarloBook = load_workbook('monteCarloMVinstances.xlsx')
monteCarloResultsBook = load_workbook('monteCarloMVresults.xlsx')
resultsSheet = monteCarloResultsBook['MV Results']
...
resultsSheet.cell(row=1, column=4).value = 'Route Request 1'
resultsSheet.cell(row=1, column=5).value = 'Route Request 2'
resultsSheet.cell(row=1, column=6).value = 'Route Request 3'
resultsSheet.cell(row=1, column=7).value = 'Route Request 4'
resultsSheet.cell(row=1, column=8).value = 'Profit'
resultsSheet.cell(row=1, column=9).value = 'Profit 1'
resultsSheet.cell(row=1, column=10).value = 'Profit 2'
resultsSheet.cell(row=1, column=11).value = 'Profit 3'
resultsSheet.cell(row=1, column=12).value = 'Profit 4'
resultsSheet.cell(row=1, column=13).value = 'No. of GS'
resultsSheet.cell(row=1, column=14).value = 'Start Time'
resultsSheet.cell(row=1, column=15).value = 'End Time'
resultsSheet.cell(row=1, column=16).value = 'Execution Time'
resultsSheet.cell(row=1, column=17).value = 'Time to find Best Solution'
resultsSheet.cell(row=1, column=18).value = 'Driver Time (in minutes)'
resultsSheet.cell(row=1, column=19).value = 'Driver Time (in minutes)'
resultsSheet.cell(row=1, column=20).value = 'Driver Time (in minutes)'
resultsSheet.cell(row=1, column=21).value = 'Driver Time (in minutes)'
resultsSheet.cell(row=1, column=22).value = 'Number of Drivers'
...
for i in range(186, 186 + iterations):
    ...
    driverTime = []
    driverNo = [1, 2, 3, 4]
    numberOfDrivers = np.random.randint(2, 5)  # random number between 2 and 4
    print("number of drivers is:", numberOfDrivers)
    driverNo = driverNo[0:numberOfDrivers]
    print("drivers are:", driverNo)
    noOfRequests = 100 + numberOfDrivers

    loadOfRequests = np.random.poisson(1, (noOfRequests, 1))  # poisson distribution for the load with expected value 1

    for ik in range(numberOfDrivers):
        driverTime.append(random.randint(3600, 7200))
    ...
    for ij in range(noOfRequests):
        ...
        if ij < numberOfDrivers:  # making driver's time as specified by the driverTime parameter
            while pickupTimeWindowE + driverTime[ij] > 9000:
                pickupTimeWindowE = startingTime + random.randint(eMinTimeDeviation, eMaxTimeDeviation)
        ...
        if ij < numberOfDrivers:
            deliveryTimeWindowL = pickupTimeWindowE + driverTime[ij]
    ...
    resultsSheet.cell(row=i + 2, column=8).value = finalReward - finalCost
    resultsSheet.cell(row=i + 2, column=13).value = numGS
    resultsSheet.cell(row=i + 2, column=14).value = start_time_VNS
    resultsSheet.cell(row=i + 2, column=15).value = end_time_VNS
    resultsSheet.cell(row=i + 2, column=16).value = end_time_VNS - start_time_VNS
    resultsSheet.cell(row=i + 2, column=17).value = timeToFindBestSolution
    resultsSheet.cell(row=i + 2, column=22).value = numberOfDrivers
    for ik in range(len(driverNo)):
        resultsSheet.cell(row=i + 2, column=4+ik).value = str(routeRequest[ik])
        resultsSheet.cell(row=i + 2, column=9+ik).value = routeReward[ik] - routeCost[ik]
        resultsSheet.cell(row=i + 2, column=18+ik).value = round(driverTime[ik]/60, 1)

    monteCarloBook.save('monteCarloMVinstances.xlsx')
    monteCarloResultsBook.save('monteCarloMVresults.xlsx')
