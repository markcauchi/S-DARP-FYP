...
sheetRequests = exhaustiveSearchInstances["Test Instance 1"]
...
sheetPT = bookResults.create_sheet('VNS Results')
...
for j in range(10):
    for i in range(iterations):
        ...
        sheetRequests = exhaustiveSearchInstances["Test Instance " + str(j + 1)]
        print("Test Instance " + str(j+1))
        requests = []
        for ij in range(noOfRequests):
            requestNo = sheetRequests.cell(row=ij + 2, column=1).value
            pickupNode = sheetRequests.cell(row=ij + 2, column=2).value
            deliveryNode = sheetRequests.cell(row=ij + 2, column=3).value
            pickupTimeWindowE = sheetRequests.cell(row=ij + 2, column=4).value
            pickupTimeWindowL = sheetRequests.cell(row=ij + 2, column=5).value
            deliveryTimeWindowE = sheetRequests.cell(row=ij + 2, column=6).value
            deliveryTimeWindowL = sheetRequests.cell(row=ij + 2, column=7).value
            directDistance = sheetRequests.cell(row=ij + 2, column=8).value
            reward = sheetRequests.cell(row=ij + 2, column=9).value
            load = sheetRequests.cell(row=ij + 2, column=10).value
        ...
        sheetPT.cell(row=10*j + i + 2, column=3).value = str(routeRequest)
        sheetPT.cell(row=10*j + i + 2, column=4).value = finalReward - finalCost
        sheetPT.cell(row=10*j + i + 2, column=5).value = numGS
        sheetPT.cell(row=10*j + i + 2, column=6).value = start_time_VNS
        sheetPT.cell(row=10*j + i + 2, column=7).value = end_time_VNS
        sheetPT.cell(row=10*j + i + 2, column=8).value = end_time_VNS - start_time_VNS
