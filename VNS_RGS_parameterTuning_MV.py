...
monteCarloBook = load_workbook('parameterTuningInstance_MV.xlsx')
sheetRequests = monteCarloBook["Test Instance 1"]

noOfNodes = 100
noOfRequests = 100  # 100 requests
requests = []
data_images = []
...
bookResults = load_workbook('parameterTuningResults.xlsx')
sheetPT = bookResults.create_sheet('parameterTuning_MV')

kmaxArray = [8, 10, 12]
percentArray = [0.25, 0.5, 1]
iterations = 20

for j in range(len(kmaxArray)):
    kmax = kmaxArray[j]
    print("kmax is:", kmax)

    for PI in range(len(percentArray)):
        percentNumber = percentArray[PI]
        print("percent Number is:", percentNumber)
        ...
        for i in range(iterations):
            ...
            [route, routeRequest, routeLoad, routeTimeWasted, routeTime, finalTime, finalLoad, finalCostEach,
             finalRewardEach, partiallyServedRequests, servedRequests, servedRequestsAll, finalReward, finalCost,
             dataOfImages] = initialSolution(timeMatrix, requests, driverNo, route, routeRequest, routeLoad,
                                             routeTimeWasted, routeTime, capacity, driverEndNode,
                                             partiallyServedRequests, partiallyServedRequestsAllVehicles, data_images)
            ...
            [route, routeRequest, numGS, timeToFindBestSolution] = VNS(route, routeRequest, routeTime, routeLoad,
                                                                       servedRequests, routeReward, routeCost,
            ...
            sheetPT.cell(row=j * len(percentArray) * iterations + PI * iterations + i + 2, column=1).value =\
                                                                                        kmax
            sheetPT.cell(row=j * len(percentArray) * iterations + PI * iterations + i + 2, column=2).value =\
                                                                                        percentNumber
            sheetPT.cell(row=j * len(percentArray) * iterations + PI * iterations + i + 2, column=3).value =\
                                                                                        i
            sheetPT.cell(row=j * len(percentArray) * iterations + PI * iterations + i + 2, column=4).value =\
                                                                                        str(route)
            sheetPT.cell(row=j * len(percentArray) * iterations + PI * iterations + i + 2, column=5).value =\
                                                                                        str(routeRequest)
            sheetPT.cell(row=j * len(percentArray) * iterations + PI * iterations + i + 2, column=6).value =\
                                                                                        finalReward - finalCost
            sheetPT.cell(row=j * len(percentArray) * iterations + PI * iterations + i + 2, column=7).value =\
                                                                                        numGS
            sheetPT.cell(row=j * len(percentArray) * iterations + PI * iterations + i + 2, column=8).value =\
                                                                                        start_time_VNS
            sheetPT.cell(row=j * len(percentArray) * iterations + PI * iterations + i + 2, column=9).value =\
                                                                                        end_time_VNS
            sheetPT.cell(row=j * len(percentArray) * iterations + PI * iterations + i + 2, column=10).value =\
                                                                                        end_time_VNS - start_time_VNS
            sheetPT.cell(row=j * len(percentArray) * iterations + PI * iterations + i + 2, column=11).value =\
                                                                                        timeToFindBestSolution

            bookResults.save('parameterTuningResults.xlsx')
